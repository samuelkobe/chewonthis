...
function add_link_atts_specific($atts, $item, $args) {
    ...
    } elseif ($args->theme_location === 'mobile-menu') { // Replace 'mobile-menu' with your desired menu location
        $atts['class'] = "mobile-menu-anchor";
        $dataTitle = formatAnchor($item->title);
        $atts['data-title'] = $dataTitle;
    } else {
        $atts['class'] = "";
...
function prefix_disable_gutenberg($current_status, $post_type)
{
    ...
    if ($post_type === 'testimonial' || $post_type === 'tour' || $post_type === 'vendor') return false;
...
function display_testimonial_tour_column($column_name, $post_id) {
    if ($column_name === 'testimonial_tour') {
        $tour_ids = get_field('associated_tour', $post_id);
        $tour_titles = array();
        foreach ($tour_ids as $tour_id) {
            $tour_title = get_the_title($tour_id);
            $tour_titles[] = '<a href="' . get_edit_post_link($tour_id) . '">' . $tour_title . '</a>';
        }
        echo implode(', ', $tour_titles);
    }
}

add_filter('manage_testimonial_posts_columns', 'add_testimonial_tour_column');
add_action('manage_posts_custom_column', 'display_testimonial_tour_column', 10, 2);


// Reorder columns and add a custom column to the "Vendor" post type
function add_vendor_tour_column($columns) {
    // Remove the "Date" column temporarily
    unset($columns['date']);

    // Add the "Tour" column
    $columns['vendor_tour'] = 'Tour';
...
function display_vendor_tour_column($column_name, $post_id) {
    if ($column_name === 'vendor_tour') {
        $tour_ids = get_field('associated_tour_vendor', $post_id);
        $tour_titles = array();
        foreach ($tour_ids as $tour_id) {
            $tour_title = get_the_title($tour_id);
            $tour_titles[] = '<a href="' . get_edit_post_link($tour_id) . '">' . $tour_title . '</a>';
